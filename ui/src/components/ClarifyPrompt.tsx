        ...
        <div className="flex flex-wrap gap-2">
          {options.map((opt) => (
            <button
              key={opt}
              onClick={() => onReply(opt)}
              className="px-3 py-1.5 rounded-full border border-amber-400 dark:border-amber-600 text-sm text-amber-800 dark:text-amber-300 hover:bg-amber-100 dark:hover:bg-amber-900/40 transition-colors"
            >
              {opt}
            </button>
          ))}
